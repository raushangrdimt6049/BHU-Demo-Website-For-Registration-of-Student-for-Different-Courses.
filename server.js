...
// New endpoint to download a payment receipt
app.get('/download-receipt/:orderId', async (req, res) => {
    const { orderId } = req.params;
    console.log(`Request to download receipt for order ID: ${orderId}`);

    try {
        // 1. Fetch payment data
        const paymentQuery = 'SELECT * FROM payments WHERE orderid = $1';
        const { rows } = await pool.query(paymentQuery, [orderId]);
        ...
        // 2. Generate PDF (re-using logic from email function)
        const receiptHtmlContent = `
        <!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Payment Receipt</title>
        <style>
            body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size: 14px; line-height: 1.5; color: #333; margin: 0; padding: 20px; background-color: #f9f9f9; }
            ...
            <div class="receipt-container">
                ...
                <div class="receipt-details">
                    <h3>Payment Receipt</h3>
                    <div class="detail-item"><span>Order ID:</span><span>${orderId}</span></div>
                    <div class="detail-item"><span>Payment ID:</span><span>${paymentData.paymentid}</span></div>
                    <div class="detail-item"><span>Payment Date:</span><span>${new Date(paymentData.paymentdate).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}</span></div>
                    <div class="detail-item"><span>Amount Paid:</span><span>₹ ${Number(paymentData.amount).toLocaleString('en-IN')}</span></div>
                    <div class="detail-item"><span>Course Details:</span><span>${paymentData.coursedetails}</span></div>
                    <div class="detail-item"><span>Paid By:</span><span>${paymentData.studentname}</span></div>
                </div>
                <div class="footer"><p>This is a computer-generated receipt.</p></div>
            </div>
        </body></html>`;
        ...
        // 3. Send PDF to client for download
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename=Payment_Receipt_${orderId}.pdf`);
        res.send(receiptPdfBuffer);
    } catch (error) {
        console.error(`Failed to generate receipt for order ${orderId}:`, error);
